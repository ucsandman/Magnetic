...
- **Markers** — color-coded notes (`M` at the playhead) that ride the media through blades, trims, and moves; ruler diamonds seek on click, the Inspector edits note/color, `Ctrl+;`/`Ctrl+'` navigate; agents drop and read them too
...
- **Roles** — tag clips Dialogue / Music / SFX (looped beds auto-tag as music); role stripes on the timeline, Dia/Mus/SFX mute buttons in the toolbar silence a whole role across playback and export
- **Loudness normalization** — one click measures a clip's source loudness (ffmpeg EBU R128, cached) and sets its volume to the −14 LUFS streaming standard; agents can propose it for every dialogue clip at once
- **Auto-ducking** — "Duck under dialogue" dips a music bed −12 dB wherever speech is detected (ramped, merged across short pauses so it never pumps); dips draw as darker bands on the bed and apply identically in live playback, offline render, and smart-render export
...
- **Edit-by-transcript** — local whisper.cpp transcription, word-click seek, select-words-and-cut (button or Delete), filler-word removal, transcript search; agents get the same power as a `cut_words` MCP tool that matches a spoken quote and proposes the cut
