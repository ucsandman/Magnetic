...
- **Copilot** — chat with Claude that answers *and edits*: it sees the clips, detected dead air, and transcript, and performs edits (blade, ripple delete, trim, roll, slip, move, transitions) against a working copy only — every batch previews as a ghost diff with a plain-English change list, applies as one undo step on Accept, and vanishes without a trace on Discard; there is no export tool, and every proposed sequence passes the timeline's invariant guard. Bring your own Anthropic API key (app settings, never logged) — with Rough Cut, the human+agent co-editing roadmap in action (`docs/agent-copilot-vision.md`)
