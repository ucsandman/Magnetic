...
- **Agent Access (MCP)** — Magnetic is an MCP server: Claude Code or any MCP client connects over loopback (`claude mcp add magnetic -- node scripts/magnetic-mcp.mjs`), reads the open timeline, and proposes edits that arrive as the same ghost-diff preview with a timeline banner — nothing applies until you Accept, there is no export tool over MCP, and the sidebar toggle severs all connections instantly (token-protected, 127.0.0.1 only)
